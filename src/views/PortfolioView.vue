...
const projects = [
  {
    ...
    approach: 'The Registry sits inside the World Bank\u2019s Public Sector Reform for Results programme, so continuity beats speed. I built the assigned features on Laravel with MySQL behind Nginx, keeping schema changes migration-based so each phased rollout stayed isolated \u2014 and delivered every milestone ahead of schedule.',
  ...
  },
  {
    ...
    description: 'A national educational platform launched by Ghana’s Ministry of Education and Ministry of Communications in 2024 — free, AI-powered learning for students at every level, accessible even offline. Developed by Npontu Technologies.',
    approach: 'Learners and administrators live in very different workflows, so instead of a monolithic page, I split the work into a Laravel API consumed by a Vue.js and Tailwind front end. One shared content model keeps the platform extensible — today it carries 5M+ learning archives and serves 575k+ active users, making Ghana the second country in Africa (after Egypt) to run a national digital learning infrastructure.',
  ...
  },
  {
    ...
    description: 'Scalable, loosely coupled module APIs for Kedebah, a multi-tenant AI-powered ERP built for Npontu Technologies that unifies finance, HR, project management, retail and CRM. Reduced API latency by 20% and increased throughput to support over 100 concurrent tenants.',
    approach: 'Multi-tenancy forced isolation and scale at the same time — one codebase serves multiple client companies nationwide. I decomposed it into loosely coupled module APIs across Flask and Laravel, put Redis in front of hot reads to cut API latency by ~20%, and kept relational state in PostgreSQL — sustaining 100+ concurrent tenants.',
  ...
  },
  {
    ...
    description: 'A mobile marketplace on Google Play connecting farmers, suppliers and buyers across the agricultural value chain. Reduced post-harvest losses to near-zero and expanded farmer access to agrochemical and agri-input resources for over 1,000 users.',
    approach: 'Ghana loses about $1.9B a year to post-harvest loss, so the fix had to reach farmers on phones, not desktops. I chose Flutter for one cross-platform codebase on top of a Laravel REST API, so updates ship to Android and iOS together — serving 1,000+ users while helping push post-harvest losses toward zero.',
